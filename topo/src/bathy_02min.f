...
C
C     SETUP FOR 2-minute NAVO/NRL DBDB2 DATA (extended 5deg across poles).
C
      INTEGER    IWI,JWI
      REAL*4     XFIN,YFIN,DXIN,DYIN
      PARAMETER (IWI =10800,    JWI =5701)
      PARAMETER (XFIN=0.0,      YFIN=-95.0,
     +           DXIN=1.0/30.0, DYIN=1.0/30.0)
C
C     BATHTMETRY ARRAYS.
C
...
C
      ...
      NAMELIST/TOPOG/ CTITLE,COAST,INTERP,MTYPE,JPR
C
C**********
C*
...
C
      CTITLE = 
     +  'bathymetery from 2-minute ????? global dataset'  !replace
      COAST  = 5.0
      INTERP = 0
      MTYPE  = 0
      JPR    = 8
