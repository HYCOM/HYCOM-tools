...
c --- uniform lon grid
      do j= 1,jdm
        do i= 1,idm
          ...
          elseif (2.0*plat(i,1)-plat(i,2).lt.-90.0) then
            qlat(i,j) = -90.0
          else
...
c
      ...
      do j= 1,jdm
        do i= 1,1
          ...
          qscx(i,j) = max(qscx(2,j),hmina)
          uscx(i,j) = max(uscx(2,j),hmina)
          vscx(i,j) = max(vscx(2,j),hmina)
          pscy(i,j) = max(pscy(2,j),hmina)
          qscy(i,j) = max(qscy(2,j),hmina)
          uscy(i,j) = max(uscy(2,j),hmina)
          vscy(i,j) = max(vscy(2,j),hmina)
        enddo
c ---   grid spacing only depends on j.
        do i= 2,idm
          pscx(i,j) =     pscx(1,j)
          qscx(i,j) =     qscx(1,j)
          uscx(i,j) =     uscx(1,j)
