...
c
      integer   i,j
      real      hmaxa,hmaxb,hmina,hminb,bref,bscl
      character preambl(5)*79,cline*80
c
c --- read in a hycom topography file,
c --- change any values deeper than bref to bref+bscl*(b-bref)
...
c
      ...
      allocate( dh(idm,jdm) )
c
c --- read in the required scaling parameters
c
      read(5,*) bref,bscl
c
c --- read in a hycom topography file,
...
c
      ...
      if     (i.eq.0) then
        write(preambl(5),        '(a,f7.4,a,f9.2,a)')
     &   'Scaled by',bscl,' below',bref,' m.'
      elseif (i.le.32) then
        write(preambl(5)(i+5:79),'(a,f7.4,a,f9.2,a)')
     &   'Scaled by',bscl,' below',bref,' m.'
      endif
c
      write(6, *)       
...
c
      do j= 1,jdm
        do i= 1,idm
          if     (dh(i,j).lt.2.0**99) then
            if     (dh(i,j).gt.bref) then
              dh(i,j) = bref + bscl*(dh(i,j) - bref)
            endif
            ip(i,j) = 1
          else
            ip(i,j) = 0
      ...
      enddo !j
c
c --- write out the shrunk hycom topography file,
c
      call zaiopn('new', 61)
