...
C
C     SETUP FOR 1/75 degree Antarctic DATA
C
      INTEGER    IWI,JWI
      REAL*8     XFIN,YFIN,DXIN,DYIN
      PARAMETER (IWI =27000,      JWI =2101)
      PARAMETER (DXIN=1.D0/75.D0, DYIN=1.D0/75.D0)
      PARAMETER (XFIN=0.D0,       YFIN=-88.D0)
C
C     BATHTMETRY ARRAYS.
C
...
C
      CTITLE = 
     +  "bathymetery from bedmap2 Antarctic 1/75 degree dataset"  !replace
      COAST  =  5.0
      FLAND  = -999999.0
      INTERP =  0
      MTYPE  =  0
...
C
      ...
      ELSE
        WRITE(6,'(/a/)')
     &    'error - CDF_BEDMAP is not defined'
          CALL ZHFLSH(6)
          STOP
      ENDIF
C
