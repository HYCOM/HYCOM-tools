...
c
     ...
     &                                slp(:,:)
c
c --- calculate the topography slope.
c
...
c
        ...
        do i= 1,idm
          if     (ip(i,j).eq.1) then
            ...
            if     (ip(i+1,j).eq.1) then
              slope = max(slope, abs((dh(i,j)-dh(i+1,j))/pdx(i,j)))
             endif
            if     (ip(i-1,j).eq.1) then
              slope = max(slope, abs((dh(i,j)-dh(i-1,j))/pdx(i,j)))
             endif
            if     (ip(i,j+1).eq.1) then
              slope = max(slope, abs((dh(i,j)-dh(i,j+1))/pdy(i,j)))
             endif
            if     (ip(i,j-1).eq.1) then
              slope = max(slope, abs((dh(i,j)-dh(i,j-1))/pdy(i,j)))
             endif
      ...
      enddo
c
c --- write out the percentage slope.
c
      call zaiowr(slp, ip,.true., hmina,hmaxa, 61, .false.)
      write(61,'(a,2f10.3)') 'slope:  min,max = ',hmina,hmaxa
      write(6, '(a,2f10.3)') 'slope:  min,max = ',hmina,hmaxa
      write(6, *)
      call zaiocl(61)
c
      end
