...
C
C     SETUP FOR 2-minute DATA (extended 2deg across poles).
C
C     92S+1min is at j=     1
C     90S+1min is at j=    61
C      0S-1min is at j=  2760
C      0S+1min is at j=  2761
C     90N-1min is at j=  5460
C     92N-1min is at j=  5520
C
      INTEGER    IWI,JWI,JWI90
      REAL*8     XFIN,YFIN,DXIN,DYIN
      PARAMETER (IWI =10800,    JWI =5520,  JWI90=5400)
      PARAMETER (DXIN=1.D0/30.D0,        DYIN=1.D0/30.D0)
...
C
      CTITLE = 
     +  "bathymetery from Sindhu's 2-minute global dataset"  !replace
      COAST  =  5.0
      FLAND  = -999999.0
      INTERP =  0
      MTYPE  =  0
...
C
      CFILE = ' '
      CALL GETENV('CDF_SINDHU',CFILE)
      IF     (CFILE.NE.' ') THEN
        WRITE(6,*)
        WRITE(6,*) 'CDF_SINDHU = ',trim(CFILE)
        CALL ZHFLSH(6)
        ! open NetCDF file
     ...
     &                             ncVID))
C
        call ncheck(nf90_get_var(ncFID,ncVID,BTHYIN))
        write(6,*) 'IZ.1,1 = ',BTHYIN(  1,    1)
        write(6,*) 'IZ.N,M = ',BTHYIN(IWI,JWI90)
C
...
C
      ...
      ELSE
        WRITE(6,'(/a/)')
     &    'error - CDF_SINDHU is not defined'
          CALL ZHFLSH(6)
          STOP
      ENDIF
C
