...
C
C     SETUP FOR 7.5 arc-second DATA (extended 2deg across poles).
C
C     92S+7.5sec is at j=     1
C     90S+7.5sec is at j=   961
C      0S-7.5sec is at j= 44160
C      0S+7.5sec is at j= 44161
C     90N-7.5sec is at j= 87360
C     92N-7.5sec is at j= 88320
C
      INTEGER    IWI,JWI,JWI90
      REAL*8     XFIN,YFIN,DXIN,DYIN
      PARAMETER (IWI =172800, JWI90=86400, JWI =JWI90+4*480)
      PARAMETER (DXIN=1.D0/480.D0,         DYIN=1.D0/480.D0)
      PARAMETER (XFIN=-180.D0+0.5D0*DXIN , YFIN=-92.D0+0.5D0*DYIN)
C
C     BATHTMETRY ARRAYS.
C
      INTEGER,   ALLOCATABLE :: IP(:,:)
      INTEGER*1, ALLOCATABLE :: BTHY90(:,:)
...
C        NetCDF:      GLOBAL LAND/SEA MASK (CDF_STAR).
C     OUTPUT:
C        ON UNIT 61A: HYCOM LAND/SEA MASK FOR THE SPECIFIED REGION.
C
C 5) Mikelsons et al, Global land mask for satellite ocean color remote sensing,
C    Remote Sens. Environ., 257, 112356 (2021).
C    https://doi.org/10.1016/j.rse.2021.112356
C    https://data.mendeley.com/datasets/9r93m9s7cw/1
C*
C**********
C
...
C
      CFILE = ' '
      CALL GETENV('CDF_STAR',CFILE)
      IF     (CFILE.NE.' ') THEN
        WRITE(6,*)
        WRITE(6,*) 'CDF_STAR = ',trim(CFILE)
        CALL ZHFLSH(6)
        ! open NetCDF file
...
C
        ...
        write(6,*) 'BATHY.N/2,M/2 = ',BTHY90(IWI/2,JWI90/2)
...
C
        ...
        DHMAX = -1.e30
        DO J= 1,JWI90
          JJ = JWI90+1-J  !STAR from 90 to -90 in lat
          DO I= 1,IWI
            BTHYI(I0+I,J+J0) = BTHY90(I,JJ)  !1=ocean,0=land
...
C
      ...
      ELSE
        WRITE(6,'(/ a /)')
     &    'environment variable CDF_STAR must be defined'
        CALL ZHFLSH(6)
        STOP
      ENDIF
C
...
C
      ...
      STOP
C
 6200 FORMAT(/ 1X,'MIN,MAX I COORDS = ',F10.3,',',F10.3 
     +       / 1X,'MIN,MAX J COORDS = ',F10.3,',',F10.3 /)
