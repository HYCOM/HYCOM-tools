...
c
     ...
     &            a_in(i,j).le.0.0        ) then
        ...
        enddo
      enddo
*     write(6,*) 'sum(m_in)  = ',sum(m_in(:,:))
*     write(6,*) 'sum(m_sm)  = ',sum(m_sm(:,:))
c
...
c
      do j= 1,jdm
        iv_sm(j,1) = idm
        do i= 1,idm
          if     (m_sm(i,j).eq.2) then  !1st filled point in j-row
            iv_sm(j,1) = 1  !default
            do ii= i-1,2,-1
              if     (m_sm(ii,j).eq.1) then
                iv_sm(j,1) = ii  !nearest ocean to 1st filled point
                exit
              endif
            enddo !ii
            exit
        ...
        do i= idm,iv_sm(j,1),-1
          if     (m_sm(i,j).eq.2) then  !last filled point in j-row
            iv_sm(j,2) = idm  !default
            do ii= i+1,idm-1
              if     (m_sm(ii,j).eq.1) then
                iv_sm(j,2) = ii  !nearest ocean to last filled point
                exit
              endif
            enddo !ii
            if     (iv_sm(j,1).eq.1 .or. iv_sm(j,2).eq.idm) then
              iv_sm(j,1) =   1  !full row
              iv_sm(j,2) = idm  !full row
            endif
*       write(6,'(a,3i6)') 'j,iv_sm.1,.2 = ',j,iv_sm(j,1),iv_sm(j,2)
            exit
          endif
        enddo
      enddo
...
*     write(6,'(a,4i6)') 'if,il,jf,jl = ',if_sm,il_sm,jf_sm,jl_sm
c
...
c
      integer i,ii,ip0,ip1,ipass,j,jj,ki,kj,nleft,nup,nup0
      real    sa,ss
c
      logical lfirst
      real    s(-1:1,-1:1)
...
c
      mm( : , : ,0) = 0
      mm(1:m,1:n,0) = mask
      mm(0,  1:n,0) = mask(m,:)  !assume periodic
      mm(m+1,1:n,0) = mask(1,:)  !assume periodic
      mm( : , : ,1) = mm(:,:,0)
c
          ...
          do i= iv(j,1),iv(j,2)
            if     (mm(i,j,ip0).ne.1) then  !land
              ...
              do kj= -1,1
                jj = j+kj
                do ki= -1,1
                  ii = mod(i+ki+m-1,m)+1  !assume periodic
...
*               endif
     ...
     &      ' points, with',nleft,' still to fill'
          call flush(6)
*         if     (nup.eq.0 .and. nup0.ne.0) then !debugging printout
*           do j= jf,jl
*             do i= iv(j,1),iv(j,2)
...
*         endif  !nup0
        ...
        mm(0,  1:n,:) = mm(m,1:n,:)  !assume periodic
        mm(m+1,1:n,:) = mm(1,1:n,:)  !assume periodic
      enddo  ! ipass=1,...
      if     (nleft.ne.0) then
        if     (lfirst) then
          write(6,'(/a,i6,a/a/a/)')
