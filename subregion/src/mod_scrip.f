...
!     new routine, based on remap
      subroutine remap_count(dst_count, map_wts, dst_add, src_add, 
     &                       src_count)

!-----------------------------------------------------------------------
!
!     counts the source and destination elements used when
!     remapping based on the provided weights computed elsewhere
!     first order remapping only
!
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
      ...
      real (kind=dbl_kind), dimension(:), intent(inout) ::
     &     dst_count,    ! mask for remapped field on destination grid
     &     src_count     ! mask for source field to be remapped
...
!-----------------------------------------------------------------------
      ...
      do n=1,size(dst_add)
        if (map_wts(1,n).ne.zero) then
          dst_count(dst_add(n)) = dst_count(dst_add(n)) + one
          src_count(src_add(n)) = src_count(src_add(n)) + one
        endif
      end do

!-----------------------------------------------------------------------

      end subroutine remap_count
