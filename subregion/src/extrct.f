...
c
      subroutine extrct_pa(work,n,m,io,jo,array,no,mo)
      implicit none
c
      integer n,m,io,jo,no,mo
      real    work(n,m),array(no,mo)
...
c --- array = work(io:io+no-1,jo:jo+mo-1)
c
c --- for pang on the p-grid of global domains with arctic bi-polar patch.
c --- it will also work for closed and near-global domains.
c
      integer i,iw,j,jw
...
c
      do j=m-jo+2,mo  ! arctic patch
        ...
        do i=1,no
          iw = n-mod(io+i-2,n)
          if     (work(iw,jw).le.0.0) then
            array(i,j)=work(iw,jw) + pi
          else
            array(i,j)=work(iw,jw) - pi
          endif
        enddo
