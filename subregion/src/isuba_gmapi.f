...
c
     ...
     &                        deg2rad,dist,dist_max,
     &                        plat_max,plat_max_dm,
     &                        plat_min,plat_min_dm,
     &                        dx,dy,qdx,xp,yp
...
*     do j= 1,jdm_out
      ...
      enddo
      if     (max(idm_out,jdm_out).le.6) then
        lperiod(2) = .true.  !1-D domain
      endif
      if     (lperiod(2)) then
...
c     
     ...
     &                    dist_max )
      ...
      plat_max    = maxval(plat_in(:,:))
      plat_min    = minval(plat_in(:,:))
      plat_max_dm = plat_max + dist_max
      plat_min_dm = plat_min - dist_max
      write(6,'(a,3f9.3)')
...
c     
      ...
      do jj= 1,jdm_out
        if     (laxis(2)) then
          if     (plat_out(1,jj).le.plat_min_dm) then
            i_out(:,jj) = 1
            j_out(:,jj) = 1
            x_out(:,jj) = spval
            y_out(:,jj) = spval
            write(6,'(a,i5,2f9.2)')
     &        'jj,plat_out.1,plat_min_dm = ',
     &        jj,plat_out(1,jj),plat_min_dm
            call flush(6)
            cycle
          elseif (plat_min.gt.dist_max-90.0 .and.
     &            plat_out(1,jj).lt.plat_min     ) then
c           input domain close enough to south pole
            write(6,'(a,i5,2f9.2)')
     &        'jj,plat_out.1,plat_min    = ',
     &        jj,plat_out(1,jj),plat_min
            call flush(6)
            plat_out(:,jj) = plat_min
          endif
          if     (plat_out(1,jj).ge.plat_max_dm) then
            i_out(:,jj) = 1
            j_out(:,jj) = jdm
            x_out(:,jj) = spval
            y_out(:,jj) = spval
            write(6,'(a,i5,2f9.2)')
     &        'jj,plat_out.1,plat_max = ',jj,plat_out(1,jj),plat_max_dm
            call flush(6)
            cycle
          elseif (plat_max.gt.90.0-dist_max .and.
     &            plat_out(1,jj).gt.plat_max     ) then
c           input domain close enough to north pole
            write(6,'(a,i5,2f9.2)')
     &        'jj,plat_out.1,plat_max    = ',
     &        jj,plat_out(1,jj),plat_max
            call flush(6)
            plat_out(:,jj) = plat_max
          endif
        else !not rectilinear
          if     (maxval(plat_out(:,jj)).le.plat_min_dm) then
            i_out(:,jj) = 1
            j_out(:,jj) = 1
            x_out(:,jj) = spval
            y_out(:,jj) = spval
     ...
     &         jj,maxval(plat_out(:,jj)),plat_min_dm
            call flush(6)
            cycle
          endif
          if     (minval(plat_out(:,jj)).ge.plat_max_dm) then
            i_out(:,jj) = 1
            j_out(:,jj) = jdm
            x_out(:,jj) = spval
            y_out(:,jj) = spval
     ...
     &         jj,minval(plat_out(:,jj)),plat_max_dm
            call flush(6)
            cycle
          endif
        endif !laxis(2):else
        if     (jj1.eq.0) then
          jj1 = jj  !1st significant row
...
c
          if     (plat_min.gt.dist_max-90.0 .and.
     &            plat_out(ii,jj).lt.plat_min    ) then
c           input domain close enough to south pole
            if     (ldebug) then
              write(6,'(a,2i5,2f9.2)')
     &          'ii,jj,plat_out,plat_min = ',
     &          ii,jj,plat_out(ii,jj),plat_min
              call flush(6)
            endif !ldebug
            plat_out(ii,jj) = plat_min
          elseif (plat_max.gt.90.0-dist_max .and.
     &            plat_out(ii,jj).gt.plat_max    ) then
c           input domain close enough to north pole
            if     (ldebug) then
              write(6,'(a,2i5,2f9.2)')
     &          'ii,jj,plat_out,plat_max  = ',
     &          ii,jj,plat_out(ii,jj),plat_max
              call flush(6)
            endif !ldebug
            plat_out(ii,jj) = plat_max
          endif
c
          if     (jj.eq.jj1) then
c           
...
c
          if     (yp.le.plat_min_dm .or.
     &            yp.ge.plat_max_dm     ) then
            if     (ip.eq.idm) then
              i_out(ii,jj) = 1
            else
              i_out(ii,jj) = ip
            ...
            y_out(ii,jj) = spval
            if     (ldebug) then
              write(6,*) 'yp,min,max = ',yp,plat_min_dm,plat_max_dm
              write(6,'(a,4i5,2a)')
     &          'ii,jj,i_,j_,x_,y_      = ',
...
*     subroutine cg(x,e,it,step,t,limit,n,m,value,grad,both,pre,h)
     ...
     &              value,grad,both,pre,h, ldebug)
      implicit none
      logical ldebug
      integer i,iq,it,j,k,l,limit,m,n,na,nb,nc,nd,nboth
...
*     if ( e .le. t ) goto 660
      ee = f
      if     (ldebug) then
        write(6,'(a,i4,i6,1pg20.10)')
     &    'cg - it,both,ee =',it,nboth,ee
        call flush(6)
      endif
      if ( ee    .le. tt )      goto 660
      if ( it    .ge. limit )   goto 660
      if ( nboth .gt. limit*5 ) goto 660
      f = fa
      d = da
      a = a7*a
      call pre(h(1,2),h(1,3))
