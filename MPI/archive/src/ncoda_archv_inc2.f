...
c
      if     (flnm_u.ne."NONE") then
        allocate(uncoda(1-nbdy:idm+nbdy,1-nbdy:jdm+nbdy,kncoda+kncodz),
     &           vncoda(1-nbdy:idm+nbdy,1-nbdy:jdm+nbdy,kncoda+kncodz) )
...
c ---       allow for round-off
            do k= kkin,1,-1
              if     (dp(i,j,k).gt.onem*0.0001) then
                exit
              else
                p(i,j,k) = depths(i,j)
               dp(i,j,k) = 0.0
...
c ---           q is dp00i     when much deeper than surface fixed coordinates
     ...
     &                                    p(i,j,k-1)-dp0cum(k-1) ) )
                ...
                if    (k-1.ge.kmld .and.
     &                 p(i,j,k+1).lt.depths(i,j)-onem*0.01) then
c ---             below the mixed layer and isotop, not touching the bottom
                  do l=2,kncoda
                    if     (zz(l).gt.pk) then
