...
c
     ...
     &                 zz(99),zi(0:99),rl(99)
      real             sigma(99),thk(99),thbase,deniso,thnthk,salmin,
...
c --- full-region NCODA
c
c --- 'flnm_t' = name of ncoda temperature file
...
c
      ...
      call xctilr(p(1-nbdy,1-nbdy,1),1,kkin+1,nbdy,nbdy, halo_ps)
c
      if     (flnm_p.ne."NONE") then
        do j= 1,jj
          do i= 1,ii
...
c
     ...
     &                zz(k).le.depths(i,j)            ) then
            ...
            do k= 1,kkout
              pz(k) = pout(i,j,k+1)
              tz(k) = temp(i,j,k)  !used below zi range
              sz(k) = saln(i,j,k)  !used below zi range
              rz(k) = th3d(i,j,k)  !used below zi range
     ...
     &                         tz,pz,kkout)
              if     (i+i0.eq.itest .and. j+j0.eq.jtest) then
                write(lp,'(a,2i5)') 'remap t1 - i,j =',i+i0,j+j0
                call flush(lp)
     ...
     &                               vz,pz,kkout)
            ...
            if     (isoflg.eq.3 .and. kmld.lt.kkout) then
              rz(kmld+1) = th3d(i,j,kmld+1)
              do k= kmld+2,kkout
                rz(k) = max(th3d(i,j,k),rz(k-1))
              enddo !k
c             sample tz at rz isopycnal depths
              if     (i+i0.eq.itest .and. j+j0.eq.jtest) then
                write(lp,'(a,2i5)') 'remap tz - i,j =',i+i0,j+j0
                call flush(lp)
...
*                 th3d(i,j,k) = th3d(i,j,k)
     ...
     &                                     tz(k), sigver)
              ...
              elseif (intflg.eq.1) then  !th&S
                saln(i,j,k) = sz(k)
                th3d(i,j,k) = rz(k)
                temp(i,j,k) = TOFSIG_V(rz(k)+thbase,sz(k),sigver)
              else !th&T
                temp(i,j,k) = tz(k)
                th3d(i,j,k) = rz(k)
                saln(i,j,k) = SOFSIG_V(rz(k)+thbase,tz(k),sigver)
              endif !intflg
