...
c     amended: for dynamic storage allocation May 2011.
c
c-----------------------------------------------------------------------
c
...
c
     ...
     &        allocated(wmaxy) .or.
     &        allocated(htmp)      ) then
        write(lp,*) 'Memory Leak in zaiost!"
        call xcstop('(zaiost)')
               stop '(zaiost)'
      endif
c
...
c
      allocate(w(n2drec),stat=ias)
      if     (ias.ne.0) then
        write(lp,*) 'Error allocating w'
        call xcstop('(zaiost)')
               stop '(zaiost)'
      endif !ias
      allocate(wminy(jtdm),stat=ias)
      if     (ias.ne.0) then
        write(lp,*) 'Error allocating wminy'
        call xcstop('(zaiost)')
               stop '(zaiost)'
      endif !ias
      allocate(wmaxy(jtdm),stat=ias)
      if     (ias.ne.0) then
        write(lp,*) 'Error allocating wmaxy'
        call xcstop('(zaiost)')
               stop '(zaiost)'
      endif !ias
      allocate(htmp(idm*jdm),stat=ias)
      if     (ias.ne.0) then
        write(lp,*) 'Error allocating htmp'
        call xcstop('(zaiost)')
               stop '(zaiost)'
      endif !ias
c
...
c
      vsave4 = vland4
      vland4 = data_void(1)
      call xcaget4(w,htmp, 1)  !htmp to w on 1st processor
      vland4 = vsave4
c
      if     (mnproc.eq.1) then
