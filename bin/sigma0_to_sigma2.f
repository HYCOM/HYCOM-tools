      program sigma0_to_sigma2
      implicit none
c
c     usage:  echo dens0 saln | sigma0_to_sigma2
c
c     input:  dens0 saln
c     output: dens0 saln pot.temp dens2
c
c             dens0  = Sigma-0 potential density
c             saln   = Salinity (psu)
c
...
c
      ...
      integer ios
c
      write(6,'(a)') '#     saln  pot.temp     dens0     dens2'
      do
        read(5,*,iostat=ios) dens0,saln
        if     (ios.ne.0) then
          exit
        endif
        call tofsig0(temp,  dens0,saln)
        call  sigma2(dens2, temp, saln)
        write(6,'(4f10.3)') saln,temp,dens0,dens2
      enddo
      end
...
C
C     CALCULATE POTENTIAL TEMPERATURE FROM SIGMA-0 DENSITY AND SALINITY.
C
