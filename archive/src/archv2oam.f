...
c --- version based on layers as thickness,
c --- consistent with calc_sbo2 by Richard Gross of JPL.
c
...
c
      character*240    flnm_a,flnm_m,flnm_o, cline
...
c --- note that dp always sums to total rest depth
c --- sshnon and oneta (below) allow for non-steric SSH mass anomaly
      if     (ip(i,j).eq.1) then
        dp(i,j,k)=dp(i,j,k)*thref/9.806       ! m
...
c
      if     (max(itest,jtest).gt.0) then
        ...
        write(lp,*) 'th3d.1 = ',th3d(itest,jtest,1)
        ...
        write(lp,*) 'th3d.kk= ',th3d(itest,jtest,kkin)
...
c
      ...
      if     (ip(i,j).eq.1) then
        ...
        if     (lsteric) then
          steric(i,j)=steric(i,j)/9.806  ! m
        else
          steric(i,j)=srfht(i,j)
        endif
        sshnon(i,j)=srfht(i,j)-steric(i,j)             ! m, mass anomaly
        if     (.not.loneta) then
          oneta( i,j)=1.0 + sshnon(i,j)/p(i,j,kkin+1)  !allow for mass anomaly
        endif
      else
        srfht( i,j)=flag
        montg( i,j)=flag
...
c
c --- add steric SSH to layer 1, like calc_sbo2 by Richard Gross of JPL
c --- note that dp.1 could be negative, but this is ok
      dp(i,j,1)=dp(i,j,1)+steric(i,j)/oneta(i,j)
       p(i,j,1)=          steric(i,j)/oneta(i,j)  !was zero
 7    continue
      if     (max(itest,jtest).gt.0) then
        write(lp,*) 'srfht  = ',srfht( itest,jtest)
        write(lp,*) 'steric = ',steric(itest,jtest)
        write(lp,*) 'sshnon = ',sshnon(itest,jtest)
        write(lp,*) 'oneta  = ',oneta( itest,jtest)
        write(lp,*) 'dp.1   = ',dp(itest,jtest,1)
        write(lp,*) ' p.1   = ', p(itest,jtest,1)
        write(lp,*) ' p.2   = ', p(itest,jtest,2)
        call flush(lp)
      endif !debug
c
...
c
     ...
     &                  oneta(i,j)*0.5*(p(i,j,k)+p(i,j,k+1))
                                                   !center of layer (m)
              dradius = oneta(i,j)*dp(i,j,k)       !layer thickness (m)
              dvolume = darea * dradius            !layer volume    (m^3)
              density = th3d(i,j,k)                !layer density   (kg/m^3)
c
              if     (i.eq.itest .and. j.eq.jtest) then
                write(lp,*) '    uvp = ',  uvp
                write(lp,*) '    vvp = ',  vvp
                write(lp,*) 'bradius = ',bradius
                write(lp,*) "brad'   = ",bradius-ae
                write(lp,*) 'dradius = ',dradius
                write(lp,*) 'dvolume = ',dvolume
                write(lp,*) 'density = ',density
                write(lp,*) 'dmass   = ',density*dvolume
                call flush(lp)
              endif !debug
c
