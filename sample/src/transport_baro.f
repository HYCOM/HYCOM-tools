      program transport_baro
      use mod_trans ! HYCOM transport section archive array interface
      use mod_za    ! HYCOM array I/O interface
c
      implicit none
c
c --- extract barotropic transport sections from a sequence of HYCOM archive files.
c --- three output files: net, positive and negative transport.
c
c --- based on transport3 with kdm=1
c
...
c
     ...
     &      status='new',action='write')
      write(nt,3000) ' (net)', 1,iexpt,time_f,time_l,tranfq,ntrans
      if     (lnp) then
        open (unit=np,file=flnm_p,form='formatted',
     &        status='new',action='write')
        write(np,3000) ' (+ve)', 1,iexpt,time_f,time_l,tranfq,ntrans
      endif !lnp
      if     (lnn) then
        open (unit=nn,file=flnm_n,form='formatted',
     &        status='new',action='write')
        write(nn,3000) ' (-ve)', 1,iexpt,time_f,time_l,tranfq,ntrans
      endif !lnn
c
...
c
          ...
          if     (if(itr).ne.il(itr)) then
            k=1
              do i= 1,nntr
                ...
                uvline(i) =      baline(i)
                vpline(i) =  max(baline(i),0.0)
                vnline(i) =  min(baline(i),0.0)
              enddo
     ...
     &                ysctr*xyline(i)*tkline(i)*uvline(i)
          ...
          k=1
            write(nt,3002) itr,xtrans(k)+
     &                         ytrans(k)
          if     (lnp) then
            k=1
              write(np,3002) itr,xtranp(k)+
     &                           ytranp(k)
          endif !lnp
          if     (lnn) then
            k=1
              write(nn,3002) itr,xtrann(k)+
     &                           ytrann(k)
          endif !lnn
        enddo  ! 1:ntrans
      enddo  ! 1:narch
