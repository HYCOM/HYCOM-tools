...
c --- low memory version, same result as transport3 but probably slower.
c
      character*240     flnm,flnm_f,flnm_l,flnm_t,flnm_p,flnm_n
c
      logical          lexist,lfatal,lnp,lnn
      integer          i,iar,iexpt,j,k,narch,yrflag
      integer          iinc,it,itm,itr,jinc,jt,jtm,nntr
      real             depthi(0:1,0:1),duk,dukm1,dvk,dvkm1,
     &                 xsctr,ysctr
      real             tranfq
      double precision time_f,time_l,time
c
      integer, allocatable :: landsea(:)
      real,    allocatable :: pkm1(:,:)
c
...
c
     ...
     &                                 iar,time,trim(flnm)
        call flush(lp)
c
        xtrans2(1:kk,1:ntrans) = 0.0
        ytrans2(1:kk,1:ntrans) = 0.0
        xtranp2(1:kk,1:ntrans) = 0.0
        ytranp2(1:kk,1:ntrans) = 0.0
        xtrann2(1:kk,1:ntrans) = 0.0
        ytrann2(1:kk,1:ntrans) = 0.0
c
c ---   loop through all layers
c
        pkm1(:,:) = 0.0
        do k= 1,kk
c
        call getdat_layer(flnm, time, k, iexpt,yrflag, trcout)
c
     ...
     &                abs(jl(itr)-jf(itr)) ) + 1
          ...
          if     (if(itr).ne.il(itr)) then
            do i= 1,nntr
              ...
              depthi(1,1) = pkm1(it,jt)  + ronem*dp(it,jt, 1)
              depthi(1,0) = pkm1(it,jtm) + ronem*dp(it,jtm,1)
              dvkm1 = min( depthv(it,jt),
     &                     0.5*(pkm1(it,jt) + pkm1(it,jtm)) )
              dvk   = min( depthv(it,jt),
     &                     0.5*(depthi(1,0) + depthi(1,1)) )
              tkline(i) = max( 0.0, dvk-dvkm1 )  !thickv(k,i)
              uvline(i) =     v(it,jt,1) + baline(i)
              vpline(i) = max(v(it,jt,1) + baline(i),0.0)
              vnline(i) = min(v(it,jt,1) + baline(i),0.0)
            enddo !i
            xtrans2(k,itr) = xsctr*sum( xyline(1:nntr)*
     &                                  tkline(1:nntr)*
     &                                  uvline(1:nntr) )
            if     (xsctr.ge.0.0) then
              xtranp2(k,itr) = xsctr*sum( xyline(1:nntr)*
     &                                    tkline(1:nntr)*
     &                                    vpline(1:nntr) )
              xtrann2(k,itr) = xsctr*sum( xyline(1:nntr)*
     &                                    tkline(1:nntr)*
     &                                    vnline(1:nntr) )
            else !flip +ve and -ve
              xtranp2(k,itr) = xsctr*sum( xyline(1:nntr)*
...
c
          if     (jf(itr).ne.jl(itr)) then
            do i= 1,nntr
              ...
              depthi(1,1) = pkm1(it ,jt) + ronem*dp(it, jt,1)
              depthi(0,1) = pkm1(itm,jt) + ronem*dp(itm,jt,1)
              dukm1 = min( depthu(it,jt),
     &                     0.5*(pkm1(it,jt) + pkm1(itm,jt)) )
              duk   = min( depthu(it,jt),
     &                     0.5*(depthi(0,1) + depthi(1,1)) )
              tkline(i) = max( 0.0, duk-dukm1 )  ! thicku(k,i)
              uvline(i) =     u(it,jt,1) + baline(i)
              vpline(i) = max(u(it,jt,1) + baline(i),0.0)
              vnline(i) = min(u(it,jt,1) + baline(i),0.0)
            enddo !i
            ytrans2(k,itr) = ysctr*sum( xyline(1:nntr)*
     &                                  tkline(1:nntr)*
     &                                  uvline(1:nntr) )
            if     (ysctr.ge.0.0) then
              ytranp2(k,itr) = ysctr*sum( xyline(1:nntr)*
     &                                    tkline(1:nntr)*
     &                                    vpline(1:nntr) )
              ytrann2(k,itr) = ysctr*sum( xyline(1:nntr)*
     &                                    tkline(1:nntr)*
     &                                    vnline(1:nntr) )
            else !flip +ve and -ve
              ytranp2(k,itr) = ysctr*sum( xyline(1:nntr)*
     ...
     &              ysctr*xyline(i)*tkline(i)*uvline(i)
          ...
          endif ! jf(itr).ne.jl(itr)
        enddo  ! itr=1:ntrans
c
c ---   update interface depth
c
        pkm1(:,:) = pkm1(:,:) + ronem*dp(:,:,1)
        enddo  ! k=1:kk
c
c ---   output all the results for this archive
c
        do itr= 1,ntrans
          do k= 1,kk,10
            write(nt,3002) itr,xtrans2(k:min(k+9,kk),itr)+
     &                         ytrans2(k:min(k+9,kk),itr)
          enddo
          if     (lnp) then
            do k= 1,kk,10
              write(np,3002) itr,xtranp2(k:min(k+9,kk),itr)+
     &                           ytranp2(k:min(k+9,kk),itr)
            enddo
          endif !lnp
          if     (lnn) then
            do k= 1,kk,10
              write(nn,3002) itr,xtrann2(k:min(k+9,kk),itr)+
     &                           ytrann2(k:min(k+9,kk),itr)
            enddo
          endif !lnn
        enddo  ! 1:ntrans
      enddo  ! 1:narch
