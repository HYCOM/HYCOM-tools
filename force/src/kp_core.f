...
C
      ...
      CHARACTER*8      CVNAME
      CHARACTER*6      CNAME
      NAMELIST/AFTITL/ CTITLE,CNAME,CVNAME
...
C**********
C*
C 1)  FROM kPAR netCDF DATA, in CORE format, ON ITS NATIVE GRID, 
...
C        CVNAME - ONE  (8-CHARACTER) VARIABLE NAME (default "  kpar").
C
C     /AFTIME/
...
C ---         float T_10_MOD(TIME, LAT, LON) ;
C ---                 T_10_MOD:missing_value = -1.e+34f ;
C ---                 T_10_MOD:_FillValue = -1.e+34f ;
C ---                 T_10_MOD:long_name = "10m Air Temperature" ;
C ---                 T_10_MOD:long_name_mod = "L=1:124" ;
C ---                 T_10_MOD:history = "From ncep.t_10.T62.1948" ;
C ---                 T_10_MOD:units = "Kelvin" ;
C --- 
C --- // global attributes:
C ---                 :history = "Thu Sep 27 07:31:56 2012: ncatted -O -a
C --- bounds,LAT,d,c,LAT_bnds t_10_mod.1948-2009.nomads.nc\n",
C ---                         "Thu Sep 27 07:31:26 2012: ncks -x -v LAT_bnds
C --- t_10_mod.1948-2009.new.nc t_10_mod.1948-2009.nomads.nc\n",
C ---                         "FERRET V6.725   18-Sep-12" ;
C ---                 :Conventions = "CF-1.0" ;
C ---                 :NCO = "4.0.3" ;
C
C     THIS EXAMPLE IS FOR CVNAME="T_10_MOD".  
C     LON IS ASSUMED TO BE UNIFORM SPACED.
C     LAT IS ASSUMED TO BE UNIFORM SPACED OR GAUSSIAN.
C     TIME IS THE NUMBER OF FLUX RECORDS, AND TIME(N) IS THE DATE OF FLUX
...
C
        CALL FREAD1(KPARIN,IWI,JWI, CVNAME,
...
C
 ...
 6300 FORMAT(10X,'WRITING KPAR RECORD',I6,'  FDAY =',F10.3 /)
 6350 FORMAT(10X,'WRITING KPAR RECORD',I6,
     +           '    FDAY =',F10.3,
     +            '  FDATE =',F8.3,'/',I4 /)
...
C
      ...
      IF     (CFILE.EQ.' ') THEN
        WRITE(0,*) 'kp_core: no CDF071 environment variable'
        CALL EXIT(1)
        STOP
      ENDIF
...
C     END OF FREAD0.
      END
      SUBROUTINE FREAD1(KPARIN,IWI,JWI, CVNAME,
...
C
      call nchek('nf90_inq_varid-CVNAME',
     &            nf90_inq_varid(ncFID,TRIM(CVNAME),ncVID))
