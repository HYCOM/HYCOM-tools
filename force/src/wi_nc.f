...
C
     ...
     +                 SEAMSK
      INTEGER          IGRID,IXYROT,ISPEED,IWINDV,IUSTAR,
     +                 INTMSK,NGLOBE,IFILL,ISMTH,
     +                 INTERP,IOFILE,MODREC,IWFILE
      NAMELIST/WWFLAG/ IGRID,IXYROT,ISPEED,IWINDV,IUSTAR,
     +                 INTMSK,NGLOBE,IFILL,ISMTH,
     +                 INTERP,IOFILE,MODREC,IWFILE,
     +                 JPR
C
...
C                    =2 ; ON P GRID ONLY
C        IXYROT - WIND VECTOR ROTATION FLAG
C                    =1 ; ROTATE FROM E-WARD,N-WARD TO X-WARD,Y-WARD (DEFAULT)
C                    =2 ; DO NOT ROTATE
...
C
     ...
     &      'error - ARCTIC wind velocity output must be on the p-grid'
        ...
        IF      (IGRID.EQ.1) THEN
          IF     (LROTATE) THEN
            PREAMBL(2) = 'Streses on u and v grids, x-wards and y-wards'
          ELSE
            PREAMBL(2) = 'Streses on u and v grids, E-wards and N-wards'
          ENDIF
        ELSE
          IF     (LROTATE) THEN
            PREAMBL(2) = 'Streses on p-grid, x-wards and y-wards'
          ELSE
            PREAMBL(2) = 'Streses on p-grid, E-wards and N-wards'
          ENDIF
        ENDIF
      ELSE
     ...
     &        'error - wind velocity output must be on the p-grid'
        ...
        ELSE
          IF     (LROTATE) THEN
            PREAMBL(2) = 'Velocities on p-grid, x-wards and y-wards'
          ELSE
            PREAMBL(2) = 'Velocities on p-grid, E-wards and N-wards'
          ENDIF
        ENDIF
      ENDIF
      PREAMBL(3) = ' '
...
C
     ...
     +            TYI(IWI+5,JWI+5) )  !+5 needed for bicubc
        IF     (INTERP.EQ.1) THEN
