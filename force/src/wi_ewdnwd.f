...
C**********
C*
C 1)  ROTATE HYCOM WIND STRESS FROM X-WARD,Y-WARD TO E-WARD,N-WARD.
C
C 2)  INPUT
...
C
      READ( 10,'(A79)') PREAMBL
      PREAMBL(3) = 'rotated from x-ward to e-ward by wi_ewdnwd'
      WRITE(12,'(A79)') PREAMBL
      WRITE( 6,'(A79)') PREAMBL
C
      READ( 11,'(A79)') PREAMBL
      PREAMBL(3) = 'rotated from y-ward to n-ward by wi_ewdnwd'
...
C
     ...
     &                   SIN(-PANG(I,J))*TXIJ
        ...
        ENDDO
C
C       E-WARD AND N-WARD ARE "SCALAR" FIELDS
C
        CALL ARCUPD(TXM,  IDM,JDM, LARCTIC,.TRUE.)
        CALL ARCUPD(TYM,  IDM,JDM, LARCTIC,.TRUE.)
C
