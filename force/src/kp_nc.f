...
C        ON CDFSCRIP:   netCDF SCRIP INTERPOLATION WEIGHTS, SEE (6).
C     OUTPUT:
C        ON UNIT 10:    UNFORMATTED MODEL    kPAR FILE, SEE (7).
C
...
C
C 6)  IF INTERP=4, THE PRECALCULATED SCRIP REGRIDDING WEIGHTS FILE
...
C
C 7)  THE OUTPUT kPAR FIELDS ARE AT EVERY GRID POINT OF THE MODEL'S
C     'P' GRID.  ARRAY SIZE IS 'IDM' BY 'JDM'.
C
C 8)  SEVERAL STATISTICS ARE WRITTEN OUT IN ORDER TO CHECK THE 
...
C
C 9)  ALAN J. WALLCRAFT,  PLANNING SYSTEMS INC.,  OCTOBER 1995.
C      BASED ON EARILER VERSIONS BY SEVERAL AUTHORS.
C      UPDATED FOR NETCDF INPUT IN JULY 2010.
C      UPDATED FOR SCRIP INTERPOLATION IN JANUARY 2015.
C*
C**********
C
...
C
     ...
     +                KPARI,IWI+4,IWI+4,JWI+4, IBD, FXI,FYI,WKPAR3,WK)
        ELSE !interp.eq.4
          CALL SCRIP(KPM,   IDM,JDM,
     &               KPARIN,IWI,JWI )  !use original input array
        ENDIF
C
