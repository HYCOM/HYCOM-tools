...
C
C     SETUP FOR 0.25 DEGREE MODAS NEAR-GLOBAL CLIMATOLOGY.
C     EXTENDED FROM 80N to 90N TO ALLOW GLOBAL DOMAINS.
C
      INTEGER    IWI,JWI,JWIN
      REAL*4     XFIN,YFIN,DXIN,DYIN
      PARAMETER (IWI=1440, JWI=681, JWIN=641)
      PARAMETER (XFIN=0.0, YFIN=-80.0, DXIN=0.25, DYIN=0.25)
C
C     CLIM ARRAYS.
C
...
C
      ...
      REAL*4    QSEAIN(IWI,JWI),XSEAIN(IWI,JWIN)
...
C**********
C*
C 1)  FROM UNFORMATTED SST (OR SSH/SSS) DATA ON ITS NATIVE GRID, CREATE
C      A FORMATTED MODEL GRID SST FILE SUITABLE FOR INPUT TO THE
C      HYCOM ISOPYCNAL CLIMATOLOGY GENERATOR OVER THE GIVEN REGION.
C
C     INTERPOLATION IS PIECEWISE BILINEAR WITH CREEPFILL OF DATAVOIDS.
C
C 2)  PARAMETERS:
C
...
C        netCDF NATIVE PotT CLIM FILE (ENV.VAR. CDF_SST), SEE (5).
C        netCDF NATIVE S    CLIM FILE (ENV.VAR. CDF_SSS), SEE (5).
C     OUTPUT:
...
C
      IF     (INTERP.NE.0) THEN
        WRITE(6,*) 'ERROR - INTERP MUST BE 0'
        WRITE(6,*) 
        CALL ZHFLSH(6)
        STOP
      ENDIF
...
C     INITIALIZE OUTPUT.
C
      PREAMBL(1) = TRIM(CTITLE) // CMONTH(MONTH)
      WRITE(6,6000) 'OUTPUT:',TRIM(PREAMBL(1))
      CALL ZHFLSH(6)
C
      CALL ZAIOPN('NEW', 10)
C
...
*     PREAMBL(1) = TRIM(CTITLE) // CMONTH(MONTH)
      ...
      ELSE
        PREAMBL(2) = 'Sea Surface Height Anomaly'
      ENDIF
      PREAMBL(3) = ' '
      PREAMBL(4) = ' '
...
C
     ...
     &                             ncVID))
      ELSE
        CALL GETENV('CDF_SSH',CFILE)
        WRITE(6,*)
        WRITE(6,*) 'CDF_SSH = ',trim(CFILE)
        CALL ZHFLSH(6)
        ! open NetCDF file
        call ncheck(nf90_open(trim(CFILE), nf90_nowrite, ncFID))
        ! inquire variable ID
        call ncheck(nf90_inq_varid(ncFID,
     &                             'Dynamic_Height_0_Depth',
     &                             ncVID))
      ENDIF
      write(6,*) 
      write(6,*) 'ncVID = ',ncVid
...
C
        QSEAIN(:,:) = 0.0  !use 0.0 above 80N
C
        call ncheck(nf90_get_var(ncFID,ncVID,
     &                           XSEAIN(:,:) ))
        QSEAIN(1:IWI,1:JWIN) = XSEAIN(1:IWI,1:JWIN)

        write(6,*) 
        write(6,*) 'seain = ',QSEAIN(IWI/2,JWI/2)
        write(6,*) 
C
C     LANDFILL
C
...
C
          CALL MINMAX(TM,IDM,JDM, XMIN,XMAX)
          CALL AVERMS(TM,IDM,JDM, XAVE,XRMS)
          WRITE(6,8100) 'HSEA', XMIN,XMAX,XAVE,XRMS
C
C         DIAGNOSTIC PRINTOUT.
C
          IF     (MIN(ITEST,JTEST).GT.0) THEN
            WRITE(6,*)
            WRITE(6,'(A,2I5,A,F6.3)')
     +        'I,J =',ITEST,JTEST,
     +        '   SSH =',TM(ITEST,JTEST)
            WRITE(6,*)
            CALL ZHFLSH(6)
...
C
          CALL ZAIOWR(TM,MSK,.FALSE., XMIN,XMAX, 10, .FALSE.)
          WRITE(10,4102) 'sea surface height anomaly',XMIN,XMAX
        ENDIF  !SST:SSS:SSH
C
      CALL ZAIOCL(10)
      CLOSE( UNIT=10)
...
C
 ...
 6400 FORMAT(' SST/SSS/SSH CLIMATOLOGY COMPLETED.')
