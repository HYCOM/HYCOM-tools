...
C
C     SETUP FOR 0.125 DEGREE MODAS NEAR-GLOBAL CLIMATOLOGY.
C     EXTEND DOMAIN FROM 80N TO 90N, ASSUMING 100% ICE COVER.
C
      INTEGER    IWI,JWI
      REAL*4     XFIN,YFIN,DXIN,DYIN
*     PARAMETER (IWI=2880, JWI=1281) !80S-80N
      PARAMETER (IWI=2880, JWI=1361) !80S-90N
      PARAMETER (XFIN=0.0, YFIN=-80.0, DXIN=0.125, DYIN=0.125)
C
C     CLIM ARRAYS.
C
...
C
     ...
     &                         QSEAIN(:,1:1281), !80S-80N
     &                         (/ 1,1,1 /) ))
C
      IF     (ICTYPE.EQ.1) THEN
        QSEAIN(:,1282:JWI) = -1.8    !ice from 80N to 90N
      ELSE
        QSEAIN(:,1282:JWI) = -1.e+34 !data void
      ENDIF
C
C     LANDFILL
C
