...
C
C     SETUP FOR 1-DEGREE NOAA OI SST V2 GLOBAL CLIMATOLOGY.
C
      INTEGER    IWI,JWI
...
C
      ...
      INTEGER          ICTYPE,INTERP,YEAR1,YEAR,MONTH,ITEST,JTEST
      NAMELIST/AFFLAG/ ICTYPE,INTERP,YEAR1,YEAR,MONTH,ITEST,JTEST,JPR
C
C**********
C*
...
C
      ...
      ELSE
        PREAMBL(2) = 'Sea Ice Concentration'
      ENDIF
      PREAMBL(3) = ' '
      PREAMBL(4) = ' '
...
C     INITIALIZE CLIMS.
C
      NPAD = 4096 - MOD(IWI*JWI,4096)
      IF     (NPAD.EQ.4096) THEN
        NPAD = 0
      ENDIF
      CALL ZHOPEN(31, 'UNFORMATTED', 'OLD', -(IWI*JWI+NPAD))
C
...
C
*         IF     (MOD(J,1000).EQ.1 .OR. J.EQ.JDM) THEN
...
*    +        INT(YAMIN).LT.3 .OR. INT(YAMAX).GT.JWI+2     ) THEN
      IF     (INT(XAMIN).LT.2 .OR. INT(XAMAX).GT.IWI+2 .OR.
     +        INT(YAMIN).LT.2 .OR. INT(YAMAX).GT.JWI+2     ) THEN
        WRITE(6,9150)
        CALL ZHFLSH(6)
        STOP
      ENDIF
C
C       READ THE INPUT MONTH.
C
        READ(31,REC=12*(YEAR-YEAR1)+MONTH) QSEAIN
C
...
C
          ...
          ELSE
            WRITE(10,4103) 'seatmp',YEAR,MONTH,XMIN,XMAX
          ENDIF
        ELSE
C
C         ICE CONCENTRATION RANGE IS 0 TO 100%.
C
          DO J= 1,JDM
            DO I= 1,IDM
              TM(I,J) = MAX( 0.0, MIN( 100.0, TM(I,J) ) ) 
            ENDDO
          ENDDO
C
          CALL MINMAX(TM,IDM,JDM, XMIN,XMAX)
          CALL AVERMS(TM,IDM,JDM, XAVE,XRMS)
          WRITE(6,8100) 'ICEC', XMIN,XMAX,XAVE,XRMS
C
          IF     (MIN(ITEST,JTEST).GT.0) THEN
            WRITE(6,*)
            WRITE(6,'(A,2I5,A,F6.2)')
...
C
          CALL ZAIOWR(TM,MSK,.FALSE., XMIN,XMAX, 10, .FALSE.)
          IF     (YEAR1.EQ.0) THEN
            WRITE(10,4102) 'icecon',MONTH,XMIN,XMAX
          ELSE
            WRITE(10,4103) 'icecon',YEAR,MONTH,XMIN,XMAX
          ENDIF
        ENDIF
C
      CALL ZAIOCL(10)
...
C
 4101 FORMAT(A79)
 4102 FORMAT(2X,A,': month,range = ',I2.2,1P2E16.7) !climatology
 4103 FORMAT(2X,A,': year,month,range = ',I4.4,I3.2,1P2E16.7)
