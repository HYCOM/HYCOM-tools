...
C
C     SETUP FOR 0.125 DEGREE MODAS GLOBAL CLIMATOLOGY.
C
      INTEGER    IWI,JWI,KWI
      REAL*4     XFIN,YFIN,DXIN,DYIN
      PARAMETER (IWI=2048, JWI=1201, KWI=33)
      PARAMETER (XFIN= 36.0, DXIN=360.0/2048.0)
      PARAMETER (YFIN=-75.0, DYIN=0.125)
C
C     CLIM ARRAYS.
C
...
C     HDF I/O ARRAYS.
C
      INTEGER         IERR,IRANK,IFLIP(3),ISHAPE(3),ISLICE(2,3),NDSDS
      CHARACTER*(256) CHDFD,CHDFT,CHDFS,
     +                CHTITLE,CSUNIT,CSFORM,COORDSYS,CLABELS(3),
     +                CAUNITS(3),CAFORMS(3)
      REAL*4          SDSMIN,SDSMAX,AXES(IWI,3)
C
...
C
      ...
      INTEGER          ICTYPE,KSIGMA,INTERP,MONTH,ITEST,JTEST
      NAMELIST/AFFLAG/ ICTYPE,KSIGMA,INTERP,MONTH,ITEST,JTEST,JPR
C
C**********
C*
...
C                   =1; Sigma-0 AND POTENTIAL TEMPERATURE
...
C        HDF NATIVE Sig0 CLIM FILE, SEE (5).
C        HDF NATIVE T    CLIM FILE, SEE (5).
C        HDF NATIVE S    CLIM FILE, SEE (5).
C     OUTPUT:
...
C
C 5)  THE INPUT CLIM FIELDS, VIA HDF, ARE ON THE 'NATIVE' LAT-LON
...
C
      IF     (ICTYPE.EQ.1) THEN
        CALL GETENV('CARDENS', CHDFD)
        WRITE(6,*)
        WRITE(6,*) 'CARDENS = ',CHDFD(1:LEN_TRIM(CHDFD))
        CALL ZHFLSH(6)
        CALL SDSRINFO(3,IRANK,ISHAPE,
     +                SDSMIN,SDSMAX,CHTITLE,CSUNIT,CSFORM,COORDSYS,
     +                AXES,IWI,CLABELS,CAUNITS,CAFORMS,
     +                CHDFD,1, IERR)
        IF     (IERR.NE.0) THEN
          CALL SDSIERR(IERR,'SDSSLICE - DENS')
        ENDIF
     ...
     +          ABS(ABS(AXES(2,2)-AXES(1,2))-DYIN).GT.0.01     ) THEN
          WRITE(6,9000) 'DENS',IWI,JWI,KWI,XFIN,YFIN,DXIN,DYIN
          WRITE(6,*) 'ISHAPE = ',ISHAPE
          WRITE(6,*) 'AXES.1 = ',AXES(1,1),AXES(2,1)
          WRITE(6,*) 'AXES.2 = ',AXES(1,2),AXES(2,2),AXES(JWI,2)
          CALL ZHFLSH(6)
          STOP
        ENDIF
C
        CALL GETENV('CARTEMP', CHDFT)
        WRITE(6,*)
        WRITE(6,*) 'CARTEMP = ',CHDFT(1:LEN_TRIM(CHDFT))
        CALL ZHFLSH(6)
        CALL SDSRINFO(3,IRANK,ISHAPE,
     +                SDSMIN,SDSMAX,CHTITLE,CSUNIT,CSFORM,COORDSYS,
     +                AXES,IWI,CLABELS,CAUNITS,CAFORMS,
     ...
     +          ABS(ABS(AXES(2,1)-AXES(1,1))-DXIN).GT.0.01 .OR.
     +          ABS(ABS(AXES(2,2)-AXES(1,2))-DYIN).GT.0.01     ) THEN
          WRITE(6,9000) 'TEMP',IWI,JWI,KWI,XFIN,YFIN,DXIN,DYIN
          WRITE(6,*) 'ISHAPE = ',ISHAPE
          WRITE(6,*) 'AXES.1 = ',AXES(1,1),AXES(2,1)
     ...
     +          ABS(ABS(AXES(2,2)-AXES(1,2))-DYIN).GT.0.01     ) THEN
        ...
        ENDIF
C
        CALL GETENV('CARSALN', CHDFS)
        WRITE(6,*)
        WRITE(6,*) 'CARSALN = ',CHDFS(1:LEN_TRIM(CHDFS))
        CALL ZHFLSH(6)
        CALL SDSRINFO(3,IRANK,ISHAPE,
     +                SDSMIN,SDSMAX,CHTITLE,CSUNIT,CSFORM,COORDSYS,
     +                AXES,IWI,CLABELS,CAUNITS,CAFORMS,
     +                CHDFS,1, IERR)
        IF     (IERR.NE.0) THEN
          CALL SDSIERR(IERR,'SDSSLICE - SALN')
        ENDIF
        IF     (ISHAPE(1).NE.IWI .OR.
     +          ISHAPE(2).NE.JWI .OR.
     +          ISHAPE(3).NE.KWI .OR.
     ...
     +          ABS(ABS(AXES(2,1)-AXES(1,1))-DXIN).GT.0.01 .OR.
     +          ABS(ABS(AXES(2,2)-AXES(1,2))-DYIN).GT.0.01     ) THEN
          WRITE(6,9000) 'SALN',IWI,JWI,KWI,XFIN,YFIN,DXIN,DYIN
          WRITE(6,*) 'ISHAPE = ',ISHAPE
          WRITE(6,*) 'AXES.1 = ',AXES(1,1),AXES(2,1)
          WRITE(6,*) 'AXES.2 = ',AXES(1,2),AXES(2,2)
          CALL ZHFLSH(6)
...
C
        IF     (ICTYPE.EQ.1) THEN
          CALL SDSSLICE(RSEAIN,NDSDS,3,IRANK,ISHAPE,IFLIP,ISLICE,
     +                  SDSMIN,SDSMAX,CHTITLE,CSUNIT,CSFORM,COORDSYS,
     +                  AXES,IWI,CLABELS,CAUNITS,CAFORMS,
     +                  CHDFD,1, IERR)
          IF     (IERR.NE.0) THEN
            CALL SDSIERR(IERR,'SDSSLICE - DENS')
          ENDIF
          CALL SDSSLICE(TSEAIN,NDSDS,3,IRANK,ISHAPE,IFLIP,ISLICE,
     +                  SDSMIN,SDSMAX,CHTITLE,CSUNIT,CSFORM,COORDSYS,
     +                  AXES,IWI,CLABELS,CAUNITS,CAFORMS,
     +                  CHDFT,1, IERR)
          IF     (IERR.NE.0) THEN
     ...
     +                  CHDFD,1, IERR)
          ...
          ENDIF
          TSEAIN = 0.0
          CALL SDSSLICE(SSEAIN,NDSDS,3,IRANK,ISHAPE,IFLIP,ISLICE,
     +                  SDSMIN,SDSMAX,CHTITLE,CSUNIT,CSFORM,COORDSYS,
     +                  AXES,IWI,CLABELS,CAUNITS,CAFORMS,
     +                  CHDFS,1, IERR)
          IF     (IERR.NE.0) THEN
          ...
          IF     (KSIGMA.EQ.0) THEN
            DO J= 1,JDM
              DO I= 1,IDM
                ...
                RT(I,J) =            RM(I,J)
                IF     (KREC.EQ.1 .AND. TM(I,J).LT.-1.8) THEN
                  TM(I,J) = -1.8                   
...
C
      ...
      ELSEIF (IERR.EQ.-3) THEN
        WRITE(6,*) 'ERROR - MRANK (3)  TOO SMALL'
        WRITE(6,*)
        CALL EXIT(1)
      ELSEIF (IERR.EQ.-4) THEN
        WRITE(6,*) 'ERROR - MDAXES (IHI) TOO SMALL'
        WRITE(6,*)
        CALL EXIT(1)
