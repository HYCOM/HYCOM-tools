...
C
      ...
      PARAMETER (IWI=1440, JWI=697, KWI=80)
      PARAMETER (XFIN=0.0, YFIN=-84.0, DXIN=0.25, DYIN=0.25)
C
C     CLIM ARRAYS.
C
...
C
      INTEGER IBD(4)
      REAL*4  TSEAI(IWI+4,JWI+4),SSEAI(IWI+4,JWI+4)
...
C                   =5; IN-SITU TEMPERATURE AND SALINITY
C                   =6; IN-SITU TEMPERATURE AND SALINITY, UNSTABLE
...
C
C     IF ICTYPE=6, DON'T INFORCE STABLE DENSITY STRATIFICATION AND
...
C**********
C
      EXTERNAL THETA,P80, LINEAR, AVERMS,MINMAX
      REAL     THETA,P80
C
      REAL*4     ZERO,RADIAN
...
C
      ...
      ENDIF
C
C     CONVERT ICTYPE FROM 5 OR 6 TO 3 OR 4, FOR LEGACY CODE
C
      ICTYPE = ICTYPE - 2
C
C     GRID INPUT.
...
C
        ...
        DO 310 J= 1,JWI
          DO 311 I= 1,IWI
            SSEAI(I+2,J+2) = SSEAI2(I,J)*SCALE_F + ADD_OFF
            TINSIT         = TSEAI2(I,J)*SCALE_F + ADD_OFF
            DBAR           = P80(DEPTH,PLAT(I,J))
            TSEAI(I+2,J+2) = THETA(SSEAI(I+2,J+2),TINSIT, DBAR,0.0)
  311     CONTINUE
  310   CONTINUE
C
...
C         USE SURFACE FREEZING POINT FOR ALL DEPTHS.
C
          DO J= 1,JDM
            DO I= 1,IDM
     ...
     +                  SSEAI,IWI+4,IWI+4,JWI+4, IBD, FXI,FYI,WQSEA3,WK)
          ENDIF
C
C         USE SURFACE FREEZING POINT AS MINUMUM FOR ALL DEPTHS.
C
          DO J= 1,JDM
            DO I= 1,IDM
