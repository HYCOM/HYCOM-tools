...
C
C     SETUP FOR 1.0 DEGREE JISAO (COADS+IABP/POLES) GLOBAL CLIMATOLOGY.
C
      INTEGER    IWI,JWI
...
C**********
C*
C 1)  FROM UNFORMATTED TAIR DATA ON ITS NATIVE GRID, CREATE
C      A FORMATTED MODEL GRID TAIR FILE SUITABLE FOR INPUT TO THE
...
C        netCDF NATIVE TEMP CLIM FILE (ENV.VAR. CDF_TAIR), SEE (5).
...
C
C     ALL TAIR AND SSS FIELDS MUST BE DEFINED AT EVERY GRID POINT,
C      INCLUDING LAND AND BELOW THE OCEAN FLOOR.
C
C 6)  THE OUTPUT TAIR IS AT EVERY GRID POINT OF THE MODEL'S 'P' GRID.
...
C      OF THE ENTIRE BASIN ARE OUTPUT FOR TAIR AND FOR EACH 
...
C     INITIALIZE CLIMS.
C
        CALL GETENV('CDF_TAIR',CFILE)
        WRITE(6,*)
        WRITE(6,*) 'CDF_TAIR = ',trim(CFILE)
        CALL ZHFLSH(6)
        ! open NetCDF file
...
C
        DO 310 J= 1,JWI
          DO 311 I= 1,IWI
            IF     (QSEAIN(I,J).LT.1.E30) THEN
              QSEAI(I+2,J+2) = 100.0  !new _FillValue
            ELSE
...
C
          ...
          WRITE(10,4102) 'air temperature',XMIN,XMAX
C
      CALL ZAIOCL(10)
      CLOSE( UNIT=10)
C
...
C
 ...
 6400 FORMAT(' TAIR CLIMATOLOGY COMPLETED.')
