...
C     NAMELIST INPUT.
C
      CHARACTER*79   CTITLE1,CTITLE2
      INTEGER        IF(999),IL(999),JF(999),JL(999)
      REAL*4         EFOLD1(999),EFOLD2(999)
      NAMELIST/MASK/ CTITLE1,CTITLE2,IF,IL,JF,JL,EFOLD1,EFOLD2
C
C**********
C*
C 1)  CREATE TWO HYCOM RELAXATION MASKS IN ONE FILE
C
C 2)  PARAMETERS:
C
...
C
C     /MASK/
C        CTITLE1     - 2ND LINE OF OUTPUT PREAMBL
C        CTITLE2     - 3RD LINE OF OUTPUT PREAMBL
C                       (1ST LINE IS ALWAYS 'Relaxation Masks')
C        IF,IL,JF,JL - ARRAY BOX WHERE EFOLD RELAXATION IS APPLIED
C                       = 4*0.0; END OF BOX LIST
C        EFOLD1      - RELAXATION E-FOLDING TIME IN DAYS (1ST MASK)
C                       = 0.0; NO RELAXATION
C        EFOLD2      - RELAXATION E-FOLDING TIME IN DAYS (2ND MASK)
C                       = 0.0; NO RELAXATION
C
...
C
     ...
     +                  EFOLD1(K),RVAL,RVAL/RVMIN
          CALL ZHFLSH(6)
        ENDIF
      ENDDO
C
C     WRITE OUT THE 1ST MASK
C
      PREAMBL(1) = 'Relaxation Masks'
      PREAMBL(2) = CTITLE1
      PREAMBL(3) = CTITLE2
      PREAMBL(4) = ' '
...
C
      ...
      WRITE(21,4102) '   p-rmu',RMUMIN,RMUMAX
C
      WRITE(6, *)
      WRITE(6, 4101) PREAMBL
      WRITE(6, 4102) '   p-rmu',RMUMIN,RMUMAX
      WRITE(6, *)
      CALL ZHFLSH(6)
C
C     PRINTOUT THE 1ST MASK
C
...
C
     ...
     +                             EFOLD2(K).GT.0.0      ) THEN
          RVAL = 1.0/(EFOLD2(K)*86400.0)
          WRITE(6,6250) IF(K),IL(K),JF(K),JL(K),
     +                  EFOLD2(K),RVAL,RVAL/RVMIN
          CALL ZHFLSH(6)
        ENDIF
      ENDDO
C
C     WRITE OUT THE 2ND MASK
C
      CALL ZAIOWR(RMU,IRMU,.FALSE., RMUMIN,RMUMAX, 21, .FALSE.)
      CALL ZAIOCL(21)
C
      WRITE(21,4102) '   v-rmu',RMUMIN,RMUMAX
      CLOSE(UNIT=21)
C
      WRITE(6, *)
      WRITE(6, 4102) '   v-rmu',RMUMIN,RMUMAX
      WRITE(6, *)
      CALL ZHFLSH(6)
C
C     PRINTOUT THE MASK
...
C
 ...
 6200 FORMAT('IF,IL,JF,JL =',4I5,'  EFOLD1,RVAL =',F6.1,1PE12.3,
     +   '  (',0PF4.1,')')
 6250 FORMAT('IF,IL,JF,JL =',4I5,'  EFOLD2,RVAL =',F6.1,1PE12.3,
     +   '  (',0PF4.1,')')
